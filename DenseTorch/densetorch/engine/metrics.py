    ...
    def updateTest(self, pred, gt):
        ...
        if pred_dims == 3:
            for i in range(gt.shape[0]):  
              save_image(torch.from_numpy(pred[0,:,:]).type(torch.FloatTensor) , "./Dataset2/val/outclass1{}r{}.png".format(i,j))
              save_image(torch.from_numpy(pred[1,:,:]).type(torch.FloatTensor) , "./Dataset2/val/outclass2{}r{}.png".format(i,j))
        elif pred_dims == 4:
            for i in range(gt.shape[0]):  
              save_image(torch.from_numpy(pred[i,0,:,:]).type(torch.FloatTensor) , "./Dataset2/val/outclass1{}r{}.png".format(i,j))
              save_image(torch.from_numpy(pred[i,1,:,:]).type(torch.FloatTensor) , "./Dataset2/val/outclass2{}r{}.png".format(i,j))
        else:
            raise ValueError("{}-dimensional input is not supported".format(pred_dims))
        ...
        for i in range(gt.shape[0]):  
            j=randint(1,1000)
            save_image(torch.from_numpy(pred[i,:,:]).type(torch.FloatTensor) , "./Dataset2/val/out{}r{}.png".format(i,j))
            save_image(torch.from_numpy(gt[i,:,:]).type(torch.FloatTensor) , "./Dataset2/val/target{}r{}.png".format(i,j))
